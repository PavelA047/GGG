...
public class ParticleController extends ObjectPool<Particle> {
    public class EffectBuilder {
        ...
        public void takePowerUpEffect(float x, float y, PowerUps.Type type) {
            switch (type) {
                case MEDKIT:
                    for (int i = 0; i < 16; i++) {
...
//      Устанавливает отдельную функцию смешивания (цвет /альфа), которая будет использоваться при рендеринге спрайтов
//      (указан стандарт, не обязательно)
...
//          Задает цвет, используемый для окрашивания изображений при их добавлении в пакет
            batch.draw(oneParticle, o.getPosition().x - 8, o.getPosition().y - 8,
                    8, 8, 16, 16, scale, scale, 0);
        }
        batch.setColor(1.0f, 1.0f, 1.0f, 1.0f);
//      возврат нормального цвета (не нужен, т.к. дальше повторяется)
...
//      включение усиления
        for (int i = 0; i < activeList.size(); i++) {
            ...
            batch.setColor(lerp(o.getR1(), o.getR2(), t), lerp(o.getG1(), o.getG2(), t),
                    lerp(o.getB1(), o.getB2(), t), lerp(o.getA1(), o.getA2(), t));
//          Задает цвет, используемый для окрашивания изображений при их добавлении в пакет
//          (если бы не было строки 52 - не нужно было бы)
            batch.draw(oneParticle, o.getPosition().x - 8, o.getPosition().y - 8,
                    8, 8, 16, 16, scale, scale, 0);
        }
        batch.setBlendFunction(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA);
//      выключает усиления, включает стандарт
        batch.setColor(1.0f, 1.0f, 1.0f, 1.0f);
//      возврат нормального цвета
    }
